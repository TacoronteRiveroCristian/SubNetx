...
export default function Dashboard() {
    ...
    const fetchLatestStatus = async () => {
        try {
            ...
            console.log('Received latest status data:', data);
    ...
    const toggleMonitoring = async () => {
        try {
            ...
            if (newState) {
                ...
                // Add this line to test and diagnose a specific target
                await fetchSpecificTargetForDiagnosis(1);
            }
    ...
    // Add this helper function to diagnose the problem
    const fetchSpecificTargetForDiagnosis = async (targetId: number) => {
        try {
            console.log(`Diagnosing target ID: ${targetId}`);
            const response = await fetch(`${API_BASE_URL}/target/${targetId}/latest`);

            if (!response.ok) {
                console.error(`Error fetching target ${targetId} diagnosis:`, response.status);
                return;
            }

            const data = await response.json();
            console.log(`Target ${targetId} diagnosis data:`, data);

            // Check critical fields
            console.log('Critical fields check:');
            console.log('- min_rtt:', typeof data.min_rtt, data.min_rtt);
            console.log('- avg_rtt:', typeof data.avg_rtt, data.avg_rtt);
            console.log('- max_rtt:', typeof data.max_rtt, data.max_rtt);
            console.log('- packet_loss_percent:', typeof data.packet_loss_percent, data.packet_loss_percent);
            console.log('- icmp_details:', Array.isArray(data.icmp_details), data.icmp_details);
        } catch (error) {
